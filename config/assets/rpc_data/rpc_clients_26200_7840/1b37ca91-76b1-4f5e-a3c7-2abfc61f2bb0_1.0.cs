//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:4.0.30319.42000
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

// Source Executable: c:\windows\system32\bisrv.dll
// Interface ID: 1b37ca91-76b1-4f5e-a3c7-2abfc61f2bb0
// Interface Version: 1.0
// Client Generated: 3/4/2026 7:36:52 PM
// NtApiDotNet Version: 1.1.33

namespace rpc_1b37ca91_76b1_4f5e_a3c7_2abfc61f2bb0_1_0
{
    
    #region Marshal Helpers
    internal class _Marshal_Helper : NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer
    {
        public void Write_0(Struct_1 p0)
        {
            WriteStruct<Struct_1>(p0);
        }
        public void Write_1(Struct_2 p0)
        {
            WriteStruct<Struct_2>(p0);
        }
        public void Write_2(Union_3 p0, long p1)
        {
            WriteUnion<Union_3>(p0, p1);
        }
        public void Write_3(Struct_4 p0)
        {
            WriteStruct<Struct_4>(p0);
        }
        public void Write_4(Struct_5 p0)
        {
            WriteStruct<Struct_5>(p0);
        }
        public void Write_5(Struct_6 p0)
        {
            WriteStruct<Struct_6>(p0);
        }
        public void Write_6(Struct_7 p0)
        {
            WriteStruct<Struct_7>(p0);
        }
        public void Write_7(Struct_8 p0)
        {
            WriteStruct<Struct_8>(p0);
        }
        public void Write_8(Struct_2[] p0, long p1)
        {
            WriteConformantStructArray<Struct_2>(p0, p1);
        }
        public void Write_9(string p0, long p1)
        {
            WriteConformantVaryingString(p0, p1);
        }
        public void Write_10(string p0, long p1)
        {
            WriteConformantVaryingString(p0, p1);
        }
        public void Write_11(char[] p0, long p1)
        {
            WriteConformantArray<char>(p0, p1);
        }
        public void Write_12(sbyte[] p0, long p1)
        {
            WriteConformantArray<sbyte>(p0, p1);
        }
        public void Write_13(int[] p0)
        {
            WriteFixedPrimitiveArray<int>(p0, 2);
        }
        public void Write_14(NtApiDotNet.NtEvent p0)
        {
            WriteSystemHandle<NtApiDotNet.NtEvent>(p0);
        }
        public void Write_15(char[] p0, long p1)
        {
            WriteConformantArray<char>(p0, p1);
        }
        public void Write_16(char[] p0, long p1)
        {
            WriteConformantArray<char>(p0, p1);
        }
        public void Write_17(byte[] p0, long p1)
        {
            WriteConformantArray<byte>(p0, p1);
        }
        public void Write_18(System.Guid[] p0, long p1)
        {
            WriteConformantArrayCallback<System.Guid>(p0, new System.Action<System.Guid>(this.WriteGuid), p1);
        }
        public void Write_19(System.Guid[] p0, long p1)
        {
            WriteConformantArrayCallback<System.Guid>(p0, new System.Action<System.Guid>(this.WriteGuid), p1);
        }
        public void Write_20(char[] p0, long p1)
        {
            WriteConformantArray<char>(p0, p1);
        }
        public void Write_21(byte[] p0, long p1)
        {
            WriteConformantArray<byte>(p0, p1);
        }
        public void Write_22(System.Guid[] p0, long p1)
        {
            WriteConformantArrayCallback<System.Guid>(p0, new System.Action<System.Guid>(this.WriteGuid), p1);
        }
        public void Write_23(sbyte[] p0, long p1)
        {
            WriteConformantArray<sbyte>(p0, p1);
        }
        public void Write_24(int[] p0, long p1)
        {
            WriteConformantArray<int>(p0, p1);
        }
        public void Write_25(byte[] p0, long p1)
        {
            WriteConformantArray<byte>(p0, p1);
        }
        public void Write_26(char[] p0, long p1)
        {
            WriteConformantArray<char>(p0, p1);
        }
        public void Write_27(System.Guid[] p0, long p1)
        {
            WriteConformantArrayCallback<System.Guid>(p0, new System.Action<System.Guid>(this.WriteGuid), p1);
        }
        public void Write_28(sbyte[] p0, long p1)
        {
            WriteConformantArray<sbyte>(p0, p1);
        }
        public void Write_29(int[] p0, long p1)
        {
            WriteConformantArray<int>(p0, p1);
        }
        public void Write_30(byte[] p0, long p1)
        {
            WriteConformantArray<byte>(p0, p1);
        }
    }
    internal class _Unmarshal_Helper : NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer
    {
        public _Unmarshal_Helper(NtApiDotNet.Win32.Rpc.RpcClientResponse r) : 
                base(r.NdrBuffer, r.Handles, r.DataRepresentation)
        {
        }
        public _Unmarshal_Helper(byte[] ba) : 
                base(ba)
        {
        }
        public Struct_1 Read_0()
        {
            return ReadStruct<Struct_1>();
        }
        public Struct_2 Read_1()
        {
            return ReadStruct<Struct_2>();
        }
        public Union_3 Read_2()
        {
            return ReadStruct<Union_3>();
        }
        public Struct_4 Read_3()
        {
            return ReadStruct<Struct_4>();
        }
        public Struct_5 Read_4()
        {
            return ReadStruct<Struct_5>();
        }
        public Struct_6 Read_5()
        {
            return ReadStruct<Struct_6>();
        }
        public Struct_7 Read_6()
        {
            return ReadStruct<Struct_7>();
        }
        public Struct_8 Read_7()
        {
            return ReadStruct<Struct_8>();
        }
        public Struct_2[] Read_8()
        {
            return ReadConformantStructArray<Struct_2>();
        }
        public char[] Read_9()
        {
            return ReadConformantArray<char>();
        }
        public sbyte[] Read_10()
        {
            return ReadConformantArray<sbyte>();
        }
        public int[] Read_11()
        {
            return ReadFixedPrimitiveArray<int>(2);
        }
        public NtApiDotNet.NtEvent Read_12()
        {
            return ReadSystemHandle<NtApiDotNet.NtEvent>();
        }
        public char[] Read_13()
        {
            return ReadConformantArray<char>();
        }
        public char[] Read_14()
        {
            return ReadConformantArray<char>();
        }
        public byte[] Read_15()
        {
            return ReadConformantArray<byte>();
        }
        public System.Guid[] Read_16()
        {
            return ReadConformantArrayCallback<System.Guid>(new System.Func<System.Guid>(this.ReadGuid));
        }
        public System.Guid[] Read_17()
        {
            return ReadConformantArrayCallback<System.Guid>(new System.Func<System.Guid>(this.ReadGuid));
        }
        public char[] Read_18()
        {
            return ReadConformantArray<char>();
        }
        public byte[] Read_19()
        {
            return ReadConformantArray<byte>();
        }
        public System.Guid[] Read_20()
        {
            return ReadConformantArrayCallback<System.Guid>(new System.Func<System.Guid>(this.ReadGuid));
        }
        public sbyte[] Read_21()
        {
            return ReadConformantArray<sbyte>();
        }
        public int[] Read_22()
        {
            return ReadConformantArray<int>();
        }
        public byte[] Read_23()
        {
            return ReadConformantArray<byte>();
        }
        public char[] Read_24()
        {
            return ReadConformantArray<char>();
        }
        public System.Guid[] Read_25()
        {
            return ReadConformantArrayCallback<System.Guid>(new System.Func<System.Guid>(this.ReadGuid));
        }
        public sbyte[] Read_26()
        {
            return ReadConformantArray<sbyte>();
        }
        public int[] Read_27()
        {
            return ReadConformantArray<int>();
        }
        public byte[] Read_28()
        {
            return ReadConformantArray<byte>();
        }
    }
    #endregion
    #region Complex Types
    public struct Struct_1 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteInt16(Member0);
            m.WriteEmbeddedPointer<Struct_2[], long>(Member8, new System.Action<Struct_2[], long>(m.Write_8), Member0);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadInt16();
            Member8 = u.ReadEmbeddedPointer<Struct_2[]>(new System.Func<Struct_2[]>(u.Read_8), false);
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public short Member0;
        public NtApiDotNet.Ndr.Marshal.NdrEmbeddedPointer<Struct_2[]> Member8;
        public static Struct_1 CreateDefault()
        {
            return new Struct_1();
        }
        public Struct_1(short Member0, Struct_2[] Member8)
        {
            this.Member0 = Member0;
            this.Member8 = Member8;
        }
    }
    public struct Struct_2 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteEmbeddedPointer<string, long>(Member0, new System.Action<string, long>(m.Write_9), 65);
            m.WriteEnum16(Member8);
            m.Write_2(Member10, Member8);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadEmbeddedPointer<string>(new System.Func<string>(u.ReadConformantVaryingString), false);
            Member8 = u.ReadEnum16();
            Member10 = u.Read_2();
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 8;
        }
        public NtApiDotNet.Ndr.Marshal.NdrEmbeddedPointer<string> Member0;
        public NtApiDotNet.Ndr.Marshal.NdrEnum16 Member8;
        public Union_3 Member10;
        public static Struct_2 CreateDefault()
        {
            return new Struct_2();
        }
        public Struct_2(string Member0, NtApiDotNet.Ndr.Marshal.NdrEnum16 Member8, Union_3 Member10)
        {
            this.Member0 = Member0;
            this.Member8 = Member8;
            this.Member10 = Member10;
        }
    }
    public struct Union_3 : NtApiDotNet.Ndr.Marshal.INdrNonEncapsulatedUnion
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            throw new System.NotImplementedException();
        }
        void NtApiDotNet.Ndr.Marshal.INdrNonEncapsulatedUnion.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m, long l)
        {
            Selector = ((NtApiDotNet.Ndr.Marshal.NdrEnum16)(l));
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteEnum16(Selector);
            if ((Selector == 0))
            {
                m.WriteInt32(Arm_0);
                goto done;
            }
            if ((Selector == 1))
            {
                m.WriteInt64(Arm_1);
                goto done;
            }
            if ((Selector == 2))
            {
                m.WriteEmbeddedPointer<string, long>(Arm_2, new System.Action<string, long>(m.Write_10), 2049);
                goto done;
            }
            if ((Selector == 3))
            {
                m.Write_3(Arm_3);
                goto done;
            }
            if ((Selector == 4))
            {
                m.Write_4(Arm_4);
                goto done;
            }
            throw new System.ArgumentException("No matching union selector when marshaling Union_3");
        done:
            return;
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Selector = u.ReadEnum16();
            if ((Selector == 0))
            {
                Arm_0 = u.ReadInt32();
                goto done;
            }
            if ((Selector == 1))
            {
                Arm_1 = u.ReadInt64();
                goto done;
            }
            if ((Selector == 2))
            {
                Arm_2 = u.ReadEmbeddedPointer<string>(new System.Func<string>(u.ReadConformantVaryingString), false);
                goto done;
            }
            if ((Selector == 3))
            {
                Arm_3 = u.Read_3();
                goto done;
            }
            if ((Selector == 4))
            {
                Arm_4 = u.Read_4();
                goto done;
            }
            throw new System.ArgumentException("No matching union selector when marshaling Union_3");
        done:
            return;
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 8;
        }
        private NtApiDotNet.Ndr.Marshal.NdrEnum16 Selector;
        public int Arm_0;
        public long Arm_1;
        public NtApiDotNet.Ndr.Marshal.NdrEmbeddedPointer<string> Arm_2;
        public Struct_4 Arm_3;
        public Struct_5 Arm_4;
        public static Union_3 CreateDefault()
        {
            return new Union_3();
        }
        public Union_3(NtApiDotNet.Ndr.Marshal.NdrEnum16 Selector, int Arm_0, long Arm_1, string Arm_2, Struct_4 Arm_3, Struct_5 Arm_4)
        {
            this.Selector = Selector;
            this.Arm_0 = Arm_0;
            this.Arm_1 = Arm_1;
            this.Arm_2 = Arm_2;
            this.Arm_3 = Arm_3;
            this.Arm_4 = Arm_4;
        }
    }
    public struct Struct_4 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteInt16(Member0);
            m.WriteEmbeddedPointer<char[], long>(Member8, new System.Action<char[], long>(m.Write_11), Member0);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadInt16();
            Member8 = u.ReadEmbeddedPointer<char[]>(new System.Func<char[]>(u.Read_9), false);
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public short Member0;
        public NtApiDotNet.Ndr.Marshal.NdrEmbeddedPointer<char[]> Member8;
        public static Struct_4 CreateDefault()
        {
            return new Struct_4();
        }
        public Struct_4(short Member0, char[] Member8)
        {
            this.Member0 = Member0;
            this.Member8 = Member8;
        }
    }
    public struct Struct_5 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteInt16(Member0);
            m.WriteEmbeddedPointer<sbyte[], long>(Member8, new System.Action<sbyte[], long>(m.Write_12), Member0);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadInt16();
            Member8 = u.ReadEmbeddedPointer<sbyte[]>(new System.Func<sbyte[]>(u.Read_10), false);
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public short Member0;
        public NtApiDotNet.Ndr.Marshal.NdrEmbeddedPointer<sbyte[]> Member8;
        public static Struct_5 CreateDefault()
        {
            return new Struct_5();
        }
        public Struct_5(short Member0, sbyte[] Member8)
        {
            this.Member0 = Member0;
            this.Member8 = Member8;
        }
    }
    public struct Struct_6 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.Write_13(NtApiDotNet.Win32.Rpc.RpcUtils.CheckNull(Member0, "Member0"));
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.Read_11();
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public int[] Member0;
        public static Struct_6 CreateDefault()
        {
            Struct_6 ret = new Struct_6();
            ret.Member0 = new int[2];
            return ret;
        }
        public Struct_6(int[] Member0)
        {
            this.Member0 = Member0;
        }
    }
    public struct Struct_7 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteInt32(Member0);
            m.WriteInt32(Member4);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadInt32();
            Member4 = u.ReadInt32();
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public int Member0;
        public int Member4;
        public static Struct_7 CreateDefault()
        {
            return new Struct_7();
        }
        public Struct_7(int Member0, int Member4)
        {
            this.Member0 = Member0;
            this.Member4 = Member4;
        }
    }
    public struct Struct_8 : NtApiDotNet.Ndr.Marshal.INdrStructure
    {
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Marshal(NtApiDotNet.Ndr.Marshal.NdrMarshalBuffer m)
        {
            Marshal(((_Marshal_Helper)(m)));
        }
        private void Marshal(_Marshal_Helper m)
        {
            m.WriteInt32(Member0);
            m.WriteInt32(Member4);
            m.WriteSByte(Member8);
            m.WriteInt32(MemberC);
            m.WriteInt32(Member10);
            m.WriteInt32(Member14);
            m.WriteInt32(Member18);
        }
        void NtApiDotNet.Ndr.Marshal.INdrStructure.Unmarshal(NtApiDotNet.Ndr.Marshal.NdrUnmarshalBuffer u)
        {
            Unmarshal(((_Unmarshal_Helper)(u)));
        }
        private void Unmarshal(_Unmarshal_Helper u)
        {
            Member0 = u.ReadInt32();
            Member4 = u.ReadInt32();
            Member8 = u.ReadSByte();
            MemberC = u.ReadInt32();
            Member10 = u.ReadInt32();
            Member14 = u.ReadInt32();
            Member18 = u.ReadInt32();
        }
        int NtApiDotNet.Ndr.Marshal.INdrStructure.GetAlignment()
        {
            return 4;
        }
        public int Member0;
        public int Member4;
        public sbyte Member8;
        public int MemberC;
        public int Member10;
        public int Member14;
        public int Member18;
        public static Struct_8 CreateDefault()
        {
            return new Struct_8();
        }
        public Struct_8(int Member0, int Member4, sbyte Member8, int MemberC, int Member10, int Member14, int Member18)
        {
            this.Member0 = Member0;
            this.Member4 = Member4;
            this.Member8 = Member8;
            this.MemberC = MemberC;
            this.Member10 = Member10;
            this.Member14 = Member14;
            this.Member18 = Member18;
        }
    }
    #endregion
    #region Client Implementation
    public sealed class Client : NtApiDotNet.Win32.Rpc.RpcClientBase
    {
        public Client() : 
                base("1b37ca91-76b1-4f5e-a3c7-2abfc61f2bb0", 1, 0)
        {
        }
        private _Unmarshal_Helper SendReceive(int p, _Marshal_Helper m)
        {
            return new _Unmarshal_Helper(SendReceive(p, m.DataRepresentation, m.ToArray(), m.Handles));
        }
        public uint RBiRtSrvAddWaitableEvent(System.Guid p0, System.Guid p1, sbyte p2, NtApiDotNet.NtEvent p3)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            m.WriteGuid(p1);
            m.WriteSByte(p2);
            m.Write_14(p3);
            _Unmarshal_Helper u = SendReceive(0, m);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvAssociateWorkItem(out System.Guid p0, char[] p1, int p2, char[] p3, int p4, System.Guid p5, int p6, int p7, byte[] p8, int p9)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.Write_15(NtApiDotNet.Win32.Rpc.RpcUtils.CheckNull(p1, "p1"), p2);
            m.WriteInt32(p2);
            m.WriteReferent(p3, new System.Action<char[], long>(m.Write_16), p4);
            m.WriteInt32(p4);
            m.WriteGuid(p5);
            m.WriteInt32(p6);
            m.WriteInt32(p7);
            m.WriteReferent(p8, new System.Action<byte[], long>(m.Write_17), p9);
            m.WriteInt32(p9);
            _Unmarshal_Helper u = SendReceive(1, m);
            p0 = u.ReadGuid();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvCreateEvent(out System.Guid p0, int p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteInt32(p1);
            _Unmarshal_Helper u = SendReceive(2, m);
            p0 = u.ReadGuid();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvCreateEventForApp(out System.Guid p0, System.Guid p1, int p2, Struct_1 p3)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p1);
            m.WriteInt32(p2);
            m.Write_0(p3);
            _Unmarshal_Helper u = SendReceive(3, m);
            p0 = u.ReadGuid();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvCreateStatusStateName(System.Guid p0, out Struct_6 p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(4, m);
            p1 = u.Read_5();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvDeleteEvent(System.Guid p0)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(5, m);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvDisassociateWorkItem(System.Guid p0, sbyte p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            m.WriteSByte(p1);
            _Unmarshal_Helper u = SendReceive(6, m);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvEnumerateBrokeredEvents(System.Guid p0, int p1, out int p2, out System.Guid[] p3)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            m.WriteInt32(p1);
            _Unmarshal_Helper u = SendReceive(7, m);
            p2 = u.ReadInt32();
            p3 = u.ReadReferent<System.Guid[]>(new System.Func<System.Guid[]>(u.Read_16), false);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvEnumerateWorkItems(out System.Guid[] p0, out int p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            _Unmarshal_Helper u = SendReceive(8, m);
            p0 = u.ReadReferent<System.Guid[]>(new System.Func<System.Guid[]>(u.Read_17), false);
            p1 = u.ReadInt32();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvGetWorkItemProperties(System.Guid p0, out char[] p1, out int p2, out System.Nullable<Struct_6> p3, out System.Guid p4, out int p5, out byte[] p6, out int p7)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(9, m);
            p1 = u.ReadReferent<char[]>(new System.Func<char[]>(u.Read_18), false);
            p2 = u.ReadInt32();
            p3 = u.ReadReferentValue<Struct_6>(new System.Func<Struct_6>(u.Read_5), false);
            p4 = u.ReadGuid();
            p5 = u.ReadInt32();
            p6 = u.ReadReferent<byte[]>(new System.Func<byte[]>(u.Read_19), false);
            p7 = u.ReadInt32();
            return u.ReadUInt32();
        }
        // async
        public uint RBiRtSrvInitiatePause(System.Guid p0, System.Guid p1, int p2, sbyte p3, NtApiDotNet.NtEvent p4, out int p5)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            m.WriteGuid(p1);
            m.WriteInt32(p2);
            m.WriteSByte(p3);
            m.Write_14(p4);
            _Unmarshal_Helper u = SendReceive(10, m);
            p5 = u.ReadInt32();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvIsValidActivationTypeForEventType(int p0, int p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteInt32(p0);
            m.WriteInt32(p1);
            _Unmarshal_Helper u = SendReceive(11, m);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvQueryBrokerEventId(System.Guid p0, out Struct_6 p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(12, m);
            p1 = u.Read_5();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvQueryBrokerEventIdFromWorkItem(System.Guid p0, out Struct_6 p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(13, m);
            p1 = u.Read_5();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvRegisterWorkItem(out System.Guid p0, char[] p1, int p2, char[] p3, int p4, System.Guid p5, System.Guid[] p6, sbyte[] p7, int[] p8, int p9, int p10, byte[] p11, int p12)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.Write_15(NtApiDotNet.Win32.Rpc.RpcUtils.CheckNull(p1, "p1"), p2);
            m.WriteInt32(p2);
            m.WriteReferent(p3, new System.Action<char[], long>(m.Write_16), p4);
            m.WriteInt32(p4);
            m.WriteGuid(p5);
            m.WriteReferent(p6, new System.Action<System.Guid[], long>(m.Write_22), p9);
            m.WriteReferent(p7, new System.Action<sbyte[], long>(m.Write_23), p9);
            m.Write_24(NtApiDotNet.Win32.Rpc.RpcUtils.CheckNull(p8, "p8"), (p9 + 1));
            m.WriteInt32(p9);
            m.WriteInt32(p10);
            m.WriteReferent(p11, new System.Action<byte[], long>(m.Write_25), p12);
            m.WriteInt32(p12);
            _Unmarshal_Helper u = SendReceive(14, m);
            p0 = u.ReadGuid();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvRegisterWorkItemClsid(out System.Guid p0, System.Guid p1, char[] p2, int p3, System.Guid p4, System.Guid[] p5, sbyte[] p6, int[] p7, int p8, int p9, byte[] p10, int p11)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p1);
            m.WriteReferent(p2, new System.Action<char[], long>(m.Write_26), p3);
            m.WriteInt32(p3);
            m.WriteGuid(p4);
            m.WriteReferent(p5, new System.Action<System.Guid[], long>(m.Write_27), p8);
            m.WriteReferent(p6, new System.Action<sbyte[], long>(m.Write_28), p8);
            m.Write_29(NtApiDotNet.Win32.Rpc.RpcUtils.CheckNull(p7, "p7"), (p8 + 1));
            m.WriteInt32(p8);
            m.WriteInt32(p9);
            m.WriteReferent(p10, new System.Action<byte[], long>(m.Write_30), p11);
            m.WriteInt32(p11);
            _Unmarshal_Helper u = SendReceive(15, m);
            p0 = u.ReadGuid();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvSignalEvent(System.Guid p0)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(16, m);
            return u.ReadUInt32();
        }
        public uint RBiRtSrvStandbyBudgetGetThrottledTime(System.Nullable<System.Guid> p0, out Struct_7 p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteReferent(p0, new System.Action<System.Guid>(m.WriteGuid));
            _Unmarshal_Helper u = SendReceive(17, m);
            p1 = u.Read_6();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvStandbyBudgetGetAppEnergyUseLevel(out NtApiDotNet.Ndr.Marshal.NdrEnum16 p0)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            _Unmarshal_Helper u = SendReceive(18, m);
            p0 = u.ReadEnum16();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvStandbyBudgetGetAppEnergyUseTrend(out NtApiDotNet.Ndr.Marshal.NdrEnum16 p0)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            _Unmarshal_Helper u = SendReceive(19, m);
            p0 = u.ReadEnum16();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvStandbyBudgetGetTaskEnergyUseTrendContribution(System.Guid p0, out int p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            _Unmarshal_Helper u = SendReceive(20, m);
            p1 = u.ReadInt32();
            return u.ReadUInt32();
        }
        public uint RBiRtSrvUpdateEventParameters(System.Guid p0, Struct_8 p1)
        {
            _Marshal_Helper m = new _Marshal_Helper();
            m.WriteGuid(p0);
            m.Write_7(p1);
            _Unmarshal_Helper u = SendReceive(21, m);
            return u.ReadUInt32();
        }
    }
    #endregion
}

