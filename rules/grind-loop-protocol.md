---
description: Protocol for the grind-until-done stop hook. Teaches the agent when and how to create the task scratchpad so iterative workflows automatically resume until completion.
alwaysApply: true
---
...
- A slash command workflow (like `/lift-class`, `/full-report`) has multiple tracked steps
...
## Managing the Scratchpad

1. **Check off items** as you complete them: change `[ ]` to `[x]`
2. **When all items are done**, set Status to `DONE` (or `COMPLETE` -- both are accepted)
3. **If you finish early** or the user cancels, set Status to `DONE` to stop the loop
4. The hook deletes the scratchpad automatically once Status is `DONE`/`COMPLETE` or all items are checked
...
- [ ] CSecurityDescriptor::CheckAccess

## Status
IN_PROGRESS
```

## Example: Multi-Phase Report

```markdown
# Task: Full report for appinfo.dll

## Items
- [x] Phase 1: Module discovery and identity
- [x] Phase 2: Function classification
