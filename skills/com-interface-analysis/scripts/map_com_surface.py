...
Usage:
    python map_com_surface.py wuapi.dll --json
    python map_com_surface.py --system-wide --top 20
    python map_com_surface.py --system-wide --tier critical --json
    python map_com_surface.py --privileged-only --context medium_il_privileged --json
"""
...
def _sort_key(server: ComServer) -> tuple:
    return (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Risk-ranked COM attack surface.")
    ...
    print(f"{'=' * 100}")
    print(f"COM ATTACK SURFACE: {scope} ({len(servers)} servers)")
    print(f"{'=' * 100}")
    ...
    print(f"{'Rank':>4}  {'Risk':>8}  {'Methods':>7}  {'ServerType':<16}  {'Service':<20}  Name")
    print(f"{'-' * 4}  {'-' * 8}  {'-' * 7}  {'-' * 16}  {'-' * 20}  {'-' * 40}")
