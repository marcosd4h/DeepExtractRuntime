"""Prepare cross-module callgraph context for AI taint scanning.
...
The output includes:
  ...
  - Taint-specific enrichments per MUST_READ node: sink APIs, sink
    categories, global variable accesses, loop analysis, parameter count,
    and trust level
  - Trust boundary annotations and sink density summary
...
# ---------------------------------------------------------------------------
# Taint-specific enrichment for MUST_READ nodes
# ---------------------------------------------------------------------------

def _enrich_must_read_nodes(
    db_path: str,
    nodes: dict[str, dict],
    traversal_plan: dict,
    target_module: str,
) -> dict[str, dict]:
    """Add taint-specific metadata to MUST_READ nodes.

    For each MUST_READ function, extracts: sink APIs, sink categories,
    global variable accesses, loop analysis, parameter count, and trust level.
    """
    must_read_names: list[str] = []
    for depth_entries in traversal_plan.get("by_depth", {}).values():
        for entry in depth_entries:
            if entry["category"] == "MUST_READ":
                must_read_names.append(entry["function"])

    if not must_read_names:
        return {}

    status_message(f"Enriching {len(must_read_names)} MUST_READ nodes with taint metadata...")

    enrichments: dict[str, dict] = {}
    module_trust = classify_module_trust(db_path)

    with open_individual_analysis_db(db_path) as db:
        for func_name in must_read_names:
            matches = db.get_function_by_name(func_name)
            if not matches:
                continue
            func = matches[0]
            node_key = f"{target_module}::{func_name}"
            ...
            for xref in xrefs_raw:
                ...
                if cat:
                    sink_apis.append({
                        "api": callee,
                        "category": cat,
                        "severity": SINK_SEVERITY.get(cat, 0.3),
                    })
                    sink_categories.add(cat)

            global_vars = parse_json_safe(func.global_var_accesses) or []
            loop_data = parse_json_safe(func.loop_analysis) or {}
            loop_count = loop_data.get("loop_count", 0) if isinstance(loop_data, dict) else 0
            max_nesting = 0
            if isinstance(loop_data, dict):
                for loop in loop_data.get("loops", []):
                    depth = loop.get("nesting_depth", 0) if isinstance(loop, dict) else 0
                    max_nesting = max(max_nesting, depth)

            sig = func.function_signature or ""
            code = func.decompiled_code or ""
            param_count = infer_param_count(sig, code)

            enrichments[node_key] = {
                "sink_apis": sink_apis,
                "sink_categories": sorted(sink_categories),
                "max_sink_severity": max(
                    (s["severity"] for s in sink_apis), default=0.0,
                ),
                "global_var_accesses": global_vars,
                "loop_analysis": {
                    "loop_count": loop_count,
                    "max_nesting_depth": max_nesting,
                },
                "parameter_count": param_count,
                "trust_level": module_trust,
            }
...
# ---------------------------------------------------------------------------
# Sink density aggregation
# ---------------------------------------------------------------------------

def _aggregate_sink_density(enrichments: dict[str, dict]) -> dict[str, Any]:
    """Compute aggregate sink density across all enriched MUST_READ nodes."""
    total_sinks = 0
    by_category: dict[str, int] = {}
    max_severity = 0.0

    for enrich in enrichments.values():
        for sink in enrich.get("sink_apis", []):
            cat = sink["category"]
            by_category[cat] = by_category.get(cat, 0) + 1
            total_sinks += 1
            max_severity = max(max_severity, sink.get("severity", 0.0))

    return {
        "total_sinks": total_sinks,
        "max_severity": round(max_severity, 2),
        "by_category": by_category,
        "functions_with_sinks": sum(
            1 for e in enrichments.values() if e.get("sink_apis")
        ),
    }
...
def _build_trust_boundaries(
    ...
    """Build trust boundary annotations from module trust and entry points."""
    module_trust = classify_module_trust(db_path)

    rpc_entries = [ep for ep in entry_point_metadata if "RPC" in ep.get("entry_type", "")]
    com_entries = [ep for ep in entry_point_metadata if "COM" in ep.get("entry_type", "")]
    winrt_entries = [ep for ep in entry_point_metadata if "WINRT" in ep.get("entry_type", "")]
    ...
    if rpc_entries:
        boundaries["ipc_boundaries"].append({
            "type": "rpc",
    ...
    boundaries["cross_module_targets"] = [
        m for m in modules_involved
        if m.lower() not in {
            n.lower() for n in SYSTEM_DLLS
        }
    ]
...
def prepare_context(
    ...
    """Prepare the full callgraph context for AI taint scanning."""
    ...
    status_message("Computing taint enrichments for MUST_READ nodes...")
    enrichments = _enrich_must_read_nodes(
        db_path, all_nodes, traversal_plan, module_name,
    )

    trust_boundaries = _build_trust_boundaries(
        db_path, entry_point_metadata, modules_involved,
    )

    sink_density = _aggregate_sink_density(enrichments)

    entry_point_params: list[dict] = []
    for ep in entry_point_metadata:
    ...
    target_func = function_name or (root_functions[0] if root_functions else "")
    ...
    result: dict[str, Any] = {
        ...
        "depth": max_depth,
        "trust_level": trust_boundaries["module_trust"],
        "root_functions": root_functions,
        "entry_points": entry_point_metadata,
        "callgraph": {
            "nodes": all_nodes,
            "edges": deduped_edges,
            "depth_levels": traversal_plan["by_depth"],
            "stats": {
                "total_nodes": traversal_plan["counts"]["total"],
        ...
        "_summary": {
            ...
            "sink_density_total": sink_density["total_sinks"],
            "functions_with_sinks": sink_density["functions_with_sinks"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare cross-module callgraph context for AI taint scanning"
    )
    ...
    parser.add_argument("--workspace-dir", metavar="DIR",
                        help="Workspace run directory for step output")
    parser.add_argument("--workspace-step", metavar="STEP",
                        help="Workspace step name")
    ...
    if args.workspace_dir and args.workspace_step:
        ...
        if args.json:
            ...
            print(f"Step '{args.workspace_step}' written to {args.workspace_dir}")
    elif args.json:
        emit_json(result)
    else:
...
def _print_human_readable(result: dict) -> None:
    """Print a human-readable summary of the prepared context."""
    cg = result.get("callgraph", {})
    stats = cg.get("stats", {})
    te = result.get("taint_enrichment", {})
    sd = te.get("sink_density", {})
    tb = te.get("trust_boundaries", {})

    print(f"=== Taint Context: {result.get('module', '')} ===")
    print(f"  Target function: {result.get('target_function', '')}")
    ...
    must_by_depth = stats.get("must_read_by_depth", {})
    if must_by_depth:
    ...
    print(f"  Sink density:    {sd.get('total_sinks', 0)} sinks across "
          f"{sd.get('functions_with_sinks', 0)} functions "
          f"(max severity {sd.get('max_severity', 0):.2f})")
    ...
    if tb.get("ipc_boundaries"):
        print("  Trust boundaries:")
        for boundary in tb["ipc_boundaries"]:
            print(f"    [{boundary['type']}] {boundary['from_trust']} -> "
                  f"{boundary['to_trust']} ({boundary['entry_count']} entries)")
        print()
