...
| Privilege escalation | `/audit` (full dossier), `/trace-export` (entry to sink) | Trigger from medium-IL process, observe elevated operation | Service trigger + token check |
...
| UAF / lifetime | `/data-flow` (object lifetime tracking), `/trace-export` (free path) | Trigger free, then trigger use from a different entry | Race the free/use paths |
...
| COM privilege escalation | `/reconstruct-types <module> <class>`, `/trace-export <module> <export>` | com-interface-reconstruction, callgraph-tracer |
| Type confusion | `/verify <module> <func>`, `/reconstruct-types <module> <class>` | verify-decompiled, reconstruct-types |
| UAF / lifetime error | `/data-flow forward <module> <func>`, `/trace-export <module> <export>` | data-flow-tracer, callgraph-tracer |
