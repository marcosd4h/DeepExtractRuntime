"""Prepare cross-module callgraph context for AI logic vulnerability scanning.
...
def _load_shared_globals_by_function(db_path: str) -> dict[str, list[str]]:
    """Build function -> [global_name, ...] mapping from shared globals."""
    result = run_skill_script(
        "data-flow-tracer",
        "global_state_map.py",
    ...
    if not result.get("success"):
        status_message("data-flow-tracer unavailable: shared globals annotations skipped")
        return {}
    json_data = result.get("json_data", {})
    if not isinstance(json_data, dict):
        return {}

    func_to_globals: dict[str, list[str]] = {}
    for g in json_data.get("globals", []):
        g_name = g.get("name", g.get("address", "unknown"))
        for accessor in g.get("readers", []) + g.get("writers", []):
            fname = (
                accessor if isinstance(accessor, str)
                else accessor.get("function", accessor.get("function_name", ""))
            )
            if fname:
                func_to_globals.setdefault(fname, [])
                if g_name not in func_to_globals[fname]:
                    func_to_globals[fname].append(g_name)
    return func_to_globals


def _load_classifications(db_path: str) -> dict[str, str]:
    """Load per-function classification category."""
    result = run_skill_script(
        "classify-functions",
        "triage_summary.py",
    ...
    for func in json_data.get("functions", []):
        ...
        cat = func.get("category", func.get("classification", ""))
        if fname and cat:
            classifications[fname] = cat

    if not classifications:
        for cat_name, cat_data in json_data.get("categories", {}).items():
            if isinstance(cat_data, dict):
                for entry in cat_data.get("functions", []):
                    fname = entry if isinstance(entry, str) else entry.get("function_name", entry.get("name", ""))
                    if fname:
                        classifications[fname] = cat_name
...
def _annotate_structural(
    ...
    shared_globals_by_func: dict[str, list[str]],
    classifications: dict[str, str],
) -> int:
    """Add structural annotations to MUST_READ nodes. Returns count annotated."""
    count = 0
    for entries in traversal_plan.get("by_depth", {}).values():
        for entry in entries:
            ...
            classification = classifications.get(func_name)
            if classification:
                structural["classification"] = classification

            globals_accessed = shared_globals_by_func.get(func_name, [])
            if globals_accessed:
                structural["accesses_shared_globals"] = globals_accessed
...
def prepare_context(
    ...
    status_message("Loading structural enrichments for logic scanning...")
    shared_globals_by_func = _load_shared_globals_by_function(db_path)
    classifications = _load_classifications(db_path)
    annotated_count = _annotate_structural(
        traversal_plan, shared_globals_by_func, classifications,
    )
    ...
    result: dict[str, Any] = {
        ...
        "stats": {
            ...
            "annotated_must_read": annotated_count,
        },
        "_summary": {
            "module": module_name,
            "root_functions": root_functions,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Prepare cross-module callgraph context for AI logic vulnerability scanning"
    )
    ...
    parser.add_argument("--threat-model", metavar="PATH",
                        help="Path to pre-computed threat model JSON (avoids redundant dispatch detection)")
    ...
    if args.json:
        ...
        print()
        annotated = stats.get("annotated_must_read", 0)
        if annotated:
            print(f"  Annotated:      {annotated} MUST_READ nodes with structural data")
