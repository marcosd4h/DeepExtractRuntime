...
4. **Module Profile (`module_profile.json`)**: Generated **unconditionally** after function extraction (does not require `--generate-cpp`). Contains pre-computed module fingerprints: identity, scale, library composition (noise ratio, library tag breakdown), API surface (dangerous API categories, technology presence flags), and complexity metrics (loop counts, assembly size stats). See `module_profile_format_reference.md` for full schema.
