...
3. **Optional Report/Code Output**: When `--generate-cpp` is enabled **and at least one function has valid decompiled output**, the extractor writes `file_info.md`, `file_info.json`, `function_index.json`, and grouped `.cpp` files under `{cpp_output_dir}/{module_name}/` (or `{sqlite_db_dir}/extracted_raw_code/{module_name}/` if `--cpp-output-dir` is not provided). Functions are packed alphabetically into combined files of ~250‑300 lines each: `{module}_{ClassName}_group_{N}.cpp` for class methods, `{module}_standalone_group_{N}.cpp` for standalone functions, and `{module}_{library}_group_{N}.cpp` for library boilerplate (see [file_info_format_reference.md](file_info_format_reference.md#c-file-naming-grouped-files) for full details).
4. **Module Profile (`module_profile.json`)**: Generated **unconditionally** after function extraction (does not require `--generate-cpp`). Contains pre-computed module fingerprints: identity, scale, library composition (noise ratio, library tag breakdown), API surface (dangerous API categories, technology presence flags), complexity metrics (loop counts, assembly size stats), and security posture (ASLR/DEP/CFG, canary coverage). See `module_profile_format_reference.md` for full schema.
...
- `extracted_code/` — Generated `.cpp` files plus `file_info.md`/`file_info.json` (only when `-NoGenerateCpp` is **not** set) and `module_profile.json` (always generated)
...
- **Hierarchy**: Generates a structured directory of grouped `.cpp` files plus `file_info.md`/`file_info.json`/`function_index.json`.
- **Categorization**: Functions are classified by `::` presence (class methods vs. standalone) and library tag (WIL, STL, WRL, CRT, ETW/TraceLogging), sorted alphabetically within each category, then packed into numbered group files of ~250‑300 lines each. File naming: `{module}_{ClassName}_group_{N}.cpp`, `{module}_standalone_group_{N}.cpp`, `{module}_{library}_group_{N}.cpp`.
- **Function Index**: `function_index.json` maps every function name to its `.cpp` file, enabling O(1) lookup without scanning grouped files. See [function_index_format_reference.md](function_index_format_reference.md).
